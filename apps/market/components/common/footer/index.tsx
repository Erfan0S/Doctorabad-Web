      ...
      <div className="container">
        <div className={style.footerWrapper}>
          <div className="row">
            <div className="col-lg-4">
              ...
              <div className={style.footerAbout}>
                <p>
                  ...
                  <br/>
                  پلتفرم جامع بهداشت، درمان، آموزش و پرورش علوم پزشکی است است که از سال ۱۳۹۵ با تاکید بر نوآوری پایه‌گذاری گردید و اکنون با بیش از یکصدهزار کاربر، انتخاب اول گروه علوم‌پزشکی کشور برای رفع نیازهای روزمره است.
                </p>
              </div>
            </div>
            <div className="col-lg-4">
                ...
                <div className={style.footerLinks}>
                  <ul>
                    {footerLinks.map(({ id, href, title, action }) => (
                      <li key={id} onClick={() => (action ? action() : undefined)}>
                        {action ? <span>{title}</span> : <Link href={href}>{title}</Link>}
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="col-lg-4">
              <div className={style.footerDetail}>
                <p>تهران، میدان انقلاب اسلامی، نبش خیابان 12 فروردین، پلاک 1302 واحد 304</p>
                <a className={style.footerDetail} href={'tel:09999972750'}>09999972750</a>
                <p> شنبه تا چهارشنبه 9 تا 17 و پنج‌شنبه 9 تا 13</p>
                <a className={style.footerDetail}  href={'mailto:info@DoctorAbad.com'}>info@DoctorAbad.com</a>
              </div>
              <br/>
              <div className={style.footerSocials}>
                <ul>
                  {footerSocialMedia.map(({ id, href, name, svg, className }) => (
                    <li key={id}>
                      <div className={style.footerSocialsItemDesc}>
                        <span>{name}</span>
                      </div>
                      <Link href={href} className={className} title={name} target={'_blank'}>
                        {svg}
                      </Link>
                    </li>
                  ))}
          ...
          <div className={style.footerCopyRight}>
            <p>کلیه حقوق متعلق به شرکت سرزمین علوم دکترآباد می‌باشد. © 2024-2016 </p>
          </div>
        </div>
      </div>
    </footer>
