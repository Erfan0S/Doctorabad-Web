...
const SidebarNav = ({ isMainLogoActive }: Props) => {
  ...
  const getSideMenuData = () => {
    if (!isMobile) {
      return sidebarMenuData.map((item) => ({ ...item, disabled: !pathname.startsWith(item.href) }));
    }
    if (isMainLogoActive) return sidebarMenuData.map((e, i) => (i === 2 ? mobileMenuLogoSchema : e));

    const activeMenuIndex = sidebarMenuData.findIndex((item) => pathname.startsWith(item.href));

    return sidebarMenuData.map((item, i) =>
      i === 4 ? { ...mobileMenuLogoSchema, disabled: true } : { ...item, disabled: activeMenuIndex !== i }
    );
  };
  ...
  return (
    <div className={style.sidebarNav}>
      <ul>
        {getSideMenuData().map(({ id, title, subTitle, image, color, href, disabled, mobileTitle }) => (
          <li key={id} className={!disabled ? style.active : ''} id={String(id)}>
