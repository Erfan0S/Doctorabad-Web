...
export const useVerifyPhone = (
  ...
  const onBackSpace = (e: KeyboardEvent) => {
    ...
    if ((e.keyCode === 8 || e.code === 'Backspace' || e.which === 8) && index > 0) {
      e.preventDefault();
  ...
  const SubmitForm = () => {
    ...
    api
      ...
      .then(() => {
        ...
        queryClient.invalidateQueries({ queryKey: ['messages_count', 'user_club_info'], exact: true });
  ...
  const resendCode = async () => {
    ...
    try {
      ...
      toast('کد تایید با موفقیت ارسال شد', { type: 'success', position: 'top-left' });
  ...
  return { code, resendPeriod, goToPrevStep, onChangeCode, resendCode, submitLoading };
};
