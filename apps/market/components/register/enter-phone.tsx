...
export const EnterPhone = ({ setStep, changePhone, phone }: RegisterStepProps) => {
  ...
  const submit = async ({ phone }: Form, { setSubmitting }: FormikHelpers<Form>) => {
    setSubmitting(true);
    try {
      await api.getCsrf();
      ...
      {({ isSubmitting, errors }) => {
        return (
          <Form className={style.enterPhoneForm}>
            ...
            <Field name="phone" id="phone" placeholder="_ _ _ _ _ _ _ _ _ _ _" />
            <button
              type="submit"
