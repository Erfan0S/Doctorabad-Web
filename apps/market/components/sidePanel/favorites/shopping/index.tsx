...
const SidePanelFavoritesShopping: React.FC = () => {
  ...
  const { data, isLoading, fetchNextPage, hasNextPage } = useInfiniteQuery<Product[]>({
    queryFn: ({ pageParam }) => api.getFavoriteList(Number(pageParam)).then((res) => res.data.data),
    queryKey: ['favorite'],
    initialPageParam: 1,
    staleTime: 0,
  ...
  return (
    <InfiniteScroll
      ...
      hasMore={hasNextPage}
      useWindow={false}
      getScrollParent={() => document.getElementById('favoriteListContainer') as HTMLElement}
      loader={
      ...
      <div className={style.sidePanelFavoritesLearning}>
        {data?.pages.map((data, i) => (
          <React.Fragment key={i}>
            {data.map(({ title, id, product_pic, price_main, price_off, slug }) => (
              <Link
                key={id}
