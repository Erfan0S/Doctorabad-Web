...
const SidePanelClubDiscounts: React.FC<Props> = ({ setSingleOfferInfo }) => {
  const { data, isLoading, fetchNextPage, hasNextPage } = useInfiniteQuery<ClubOffer[]>({
    queryFn: ({ pageParam }) => api.getOffersList(Number(pageParam)).then((res) => res.data.data),
    queryKey: ['clubOffers'],
    initialPageParam: 1,
    staleTime: 0,
  ...
  return (
    <InfiniteScroll
      ...
      useWindow={false}
      getScrollParent={() => document.getElementById('clubListContainer') as HTMLElement}
      hasMore={hasNextPage}
    ...
    >
      {data?.pages.map((data, i) => (
        <React.Fragment key={i}>
          {data.map((offer) => {
            const { id, title, description, expired_at, discount_codes, coins, pic_url, customer_states } =
              offer;
            return (
              <div key={id} className={style.sidePanelClubDiscountsItem}>
                <div className={style.sidePanelClubDiscountsItemImage}>
                  <Image src={pic_url || placeHolderDataUrl} alt="drLearnImage" width={70} height={70} />
                  <span>{title}</span>
                </div>
                <div className={style.sidePanelClubDiscountsItemContent}>
                  <div className={style.sidePanelClubDiscountsItemHeader}>
                    {customer_states.map((state) => (
                      <span style={{ background: `#${state.color_code}` }} key={state.id}>
                        {state.title}
                      </span>
                    ))}
                  </div>
                  ...
                  {discount_codes.length ? (
                    <div className={style.sidePanelClubDiscountsItemFooterCode}>
                      ...
                      <button onClick={() => copyText(discount_codes[0].code, 'کد تخفیف کپی شد')}>
                    ...
                    <div className={style.sidePanelClubDiscountsItemFooter}>
                      <span>{coins} سکه</span>
                      <button onClick={() => setSingleOfferInfo(offer)}>دریافت</button>
                    </div>
                  )}
                </div>
              </div>
