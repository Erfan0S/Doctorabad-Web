...
export default async function Product({ params }: NextPageProps<{ id: string }>) {
  ...
  const { data: relatedProductList } = await api.getRelatedProducts(Number(data.id));

  return (
    <>
      <div className="col-xl-8">
        <ProductIntro productData={data} />
        <ProductTabs productData={data} relatedProductList={relatedProductList.data} />
      </div>
