...
import { ResponseType, RequestMethod, RequestOptions, RequestConfig } from './types/Request';
...
constructor(private config:RequestConfig) {
  ...
  async constructResponseSuccessData<T>(response: Response): Promise<ResponseType<T>> {
    return {
      status: response.status,
      data: (await this.interceptorsUtils.interceptorResolver(
        'responseSuccess',
        response.headers.get('Content-Type') === 'application/json' ? await response?.json() : response
      )) as T,
    };
  }

  async constructResponseFailedData(response: Response): Promise<ResponseType<any>> {
    return this.interceptorsUtils.interceptorResolver('responseFailed', {
      status: response.status,
      data: response.headers.get('Content-Type') === 'application/json' ? await response?.json() : response,
    });
  }

  protected async request<T = any>(url: string, method: RequestMethod, options: RequestOptions) {

    const response = await fetch(this.requestUtils.getRequestUrl(url, options?.params), {
      method,
