...
const SidePanelMainMenu: React.FC<SidePanelPageProps> = ({ setPage }) => {
  ...
  return (
    <div className={style.sidePanelMainMenu}>
      <SidePanelHeader setPage={setPage} title="کلبه من" onBack={() => modalActions.removeLastModal()} />
      <div className={style.sidePanelMainMenuHeader} onClick={() => setPage(SidePanelPage.PROFILE)}>
        <div className={style.sidePanelMainMenuHeaderImage}>
          <Image src={userInfo?.avatar || avatarImage} alt="clubImage" width={90} height={90} />
        </div>
        <div className={style.sidePanelMainMenuHeaderContent}>
          <span>{userInfo?.name || userInfo?.nickname || userInfo?.mobile}</span>
      ...
      <div className={style.sidePanelMainMenuWrapper}>
        {sidePanelMenuData.map(({ id, href, Icon, title, action }) => {
          const notifications = id === 6 && isSuccess && data.data.data.counter > 0 && (
            <span className={style.sidePanelMainMenuWrapperNotification}>{data.data.data.counter}</span>
          );
          return (
            <div key={id} onClick={() => (action ? action() : href && setPage(href))}>
              <Icon />
              <span>{title}</span>
              {notifications}
            </div>
