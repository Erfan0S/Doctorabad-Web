...
const SidePanelClubHistory: React.FC = () => {
  const { data, isLoading, fetchNextPage, hasNextPage } = useInfiniteQuery<ClubTransaction[]>({
    queryFn: ({ pageParam }) => api.getClubTransactionsList(Number(pageParam)).then((res) => res.data.data),
      ...
      >
        {data?.pages.map((data, i) => (
          <React.Fragment key={i}>
            {data.map(({ amount, club_plan, created_at, id, reason, transaction_type }) => {
                ...
                <div key={id} className={style.sidePanelClubHistoryItem}>
                  <div className={style.sidePanelClubHistoryItemContent}>
                    ...
                    <div className={style.sidePanelClubHistoryItemDate}>
                      {isMinusTransaction ? (
                        <span>تاریخ انقضا: {toFullPersianDateString(club_plan!.expired_at)}</span>
                      ) : (
                        ''
                      )}
                    ...
                    {isMinusTransaction ? (
                      <>
                        {club_plan?.discount_codes?.length ? (
                          <div className={style.sidePanelClubHistoryItemFooterCode}>
                            <span>{club_plan?.discount_codes[0].code}</span>
                            <button
                              onClick={(e) => {
                                copyText(club_plan?.discount_codes[0].code, 'کد تخفیف کپی شد');
                              }}
                            >
                          ...
                          <div className={style.sidePanelClubHistoryItemFooter}>
                            <span>{club_plan?.description}</span>
                          </div>
                        )}
                      </>
