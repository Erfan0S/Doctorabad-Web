...
const CreateDrProOrderButton = ({ selectedPlanId, payInfo }: Props) => {
  ...
  const onCreate = async () => {
    if (!selectedPlanId) {
      toast("لطفا ابتدا یک پلن انتخاب کنید", {
        type: "error",
        position: "top-left",
      });
      return;
    ...
    try {
      const resp =
        payInfo.paymentMethod === PaymentProviders.SNAPP_PAY
          ? await sharedApi.createDrProSnappPayOrder({
              discount_code_id:
                (payInfo as any).discountInfo?.discount_code_id ?? 0,
              id: selectedPlanId,
              use_credit: payInfo.payWithCredit ? 1 : 0,
              provider: PaymentProviders.SNAPP_PAY,
            })
          : await sharedApi.createDrProOrder({
              discount_code_id:
                (payInfo as any).discountInfo?.discount_code_id ?? 0,
              id: selectedPlanId,
              use_credit: payInfo.payWithCredit ? 1 : 0,
      ...
      toast(resp?.data?.data?.message ?? "سفارش ایجاد شد", {
        type: "success",
        position: "top-left",
      });
    } catch (err: any) {
      toast(err?.response?.data?.message ?? "خطا در ایجاد سفارش", {
        type: "error",
        position: "top-left",
      });
    } finally {
